...
function App({ onLogout }) {
  ...
  const cargarAreas = useCallback(async () => {
    ...
    } catch { console.error('Error al cargar ├íreas') }
  }, [API_URL])
  ...
  const cargarUsuarios = useCallback(async () => {
    ...
    } catch { setErrorBd('El backend de usuarios est├í inalcanzable') }
  }, [API_URL])
  ...
  const handleSubmitArea = async (e) => {
    ...
    } catch { alert('Error al guardar ├írea') }
  }
  ...
  const cambiarEstadoEntidad = async (path, id, nuevoEstado) => {
    const msg = nuevoEstado ? '┬┐Reactivar registro?' : '┬┐Desactivar (borrado l├│gico)?'
    ...
    <div className="layout">
      <aside className="sidebar">
        ...
        <nav className="sidebar-nav">
          <span className="nav-title">Gesti├│n</span>
          ...
          <button className={`nav-item ${seccionActual === 'areas' ? 'active' : ''}`} 
            onClick={() => { setSeccionActual('areas'); limpiarFormularios(); }}>├üreas</button>
      ...
      <main className="content">
        <header className="content-header">
          <h1>Gesti├│n de {seccionActual.charAt(0).toUpperCase() + seccionActual.slice(1)}</h1>
        ...
        <section className="panel">
          ...
          <div className="table-wrap">
            <table className="users-table">
              <thead>
                <tr>
                  {seccionActual === 'usuarios' && (<><th>Nombre</th><th>Correo</th><th>Rol</th><th>├ürea</th></>)}
                  {seccionActual === 'contratistas' && (<><th>Raz├│n Social</th><th>RUT</th></>)}
                  {seccionActual === 'areas' && (<><th>├ürea</th><th>Empresa</th></>)}
                  {seccionActual === 'disciplinas' && (<><th>Disciplina</th><th>├ürea</th></>)}
                  <th>Acciones</th>
                </tr>
              </thead>
<tbody>
